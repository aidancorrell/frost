//! Test fixture generator for Iceberg metadata.
//!
//! Generates realistic metadata JSON and Avro manifest files on disk for
//! integration testing. Each fixture function creates a complete table
//! directory under a given root path.
...
/// Create a healthy table fixture with normal-sized files and few snapshots.
pub fn create_healthy_table(root: &Path) {
    ...
    // Create data file placeholders.
    let data_files: Vec<_> = (0..10)
        .map(|i| {
            let name = format!("part-{:05}.parquet", i);
            let path = data_dir.join(&name);
            // Create a file with realistic size (write some bytes).
            let content = vec![0u8; 1024]; // placeholder
...
/// Create a table with small file pathology — many tiny files.
pub fn create_small_files_table(root: &Path) {
    ...
    // 200 small files (100KB each) + 10 normal files.
...
/// Create a table with snapshot bloat — too many snapshots.
pub fn create_snapshot_bloat_table(root: &Path) {
    ...
    // 250 snapshots, going back 250 days.
...
/// Create a table with orphan files — unreferenced data files in the data directory.
pub fn create_orphan_files_table(root: &Path) {
    ...
    // 15 orphan files (not referenced by any manifest).
    for i in 0..15 {
...
/// Create a table with schema drift — breaking changes between versions.
...
fn write_manifest_file(
    ...
    for (file_path, size, records) in data_files {
        let mut record = Record::new(&schema).unwrap();
        record.put("status", AvroValue::Int(1)); // added

        let union_val = apache_avro::types::Value::Union(1, Box::new(AvroValue::Long(1)));
        ...
        if let AvroSchema::Record(rec_schema) = &schema {
            ...
            if let AvroSchema::Record(_) = &df_field.schema {
                ...
                df_record.put("file_path", AvroValue::String(file_path.clone()));
    ...
    for (file_path, size, records) in delete_files {
        let mut record = Record::new(&schema).unwrap();
...
/// Write a manifest list file (Avro) with manifest references.
fn write_manifest_list_file(
    path: &Path,
    manifests: &[(String, i32, i32)], // (path, added_files_count, content)
) {
    let schema_str = r#"{
        "type": "record",
        "name": "manifest_file",
